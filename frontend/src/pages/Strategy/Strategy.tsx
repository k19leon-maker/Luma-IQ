...
function getMockContent(stepId: number, answers: Partial<AudienceAnswers>): string {
  const seg = answers.chosenSegment ?? 'Женщины 30–45 с тревожностью';
  const sub = answers.chosenSubsegment ?? 'Мамы подростков с проблемами в общении';
  switch (stepId) {
    case 1: return `Сегмент 1 — **Мамы подростков 12–17 лет**\nКогда: дочь/сын перестал общаться, закрылся\nХочу: восстановить контакт\nЧтобы: снова быть близкими\n\nСегмент 2 — **Женщины 30–45 с тревожностью**\nКогда: постоянный стресс и тревога\nХочу: успокоиться и выдохнуть\nЧтобы: жить без страха\n\nСегмент 3 — **Пары в кризисе отношений**\nКогда: частые ссоры, непонимание\nХочу: наладить отношения\nЧтобы: сохранить семью\n\nСегмент 4 — **Женщины после развода**\nКогда: только что расстались\nХочу: пережить разрыв\nЧтобы: начать новую жизнь\n\nСегмент 5 — **Руководители с синдромом самозванца**\nКогда: получили повышение\nХочу: обрести уверенность\nЧтобы: соответствовать роли\n\nСегмент 6 — **Молодые мамы с послеродовой тревогой**\nКогда: первый год после родов\nХочу: справиться с тревогой\nЧтобы: радоваться материнству\n\nСегмент 7 — **Подростки 14–18 лет**\nКогда: проблемы с самооценкой\nХочу: принять себя\nЧтобы: не страдать от мнения других\n\nСегмент 8 — **Работники в выгорании**\nКогда: нет сил и мотивации\nХочу: восстановить энергию\nЧтобы: снова любить работу\n\nСегмент 9 — **Пожилые люди с одиночеством**\nКогда: дети выросли и уехали\nХочу: найти смысл\nЧтобы: не чувствовать себя одинокими\n\nСегмент 10 — **Люди с фобиями и паническими атаками**\nКогда: паника в общественных местах\nХочу: перестать бояться\nЧтобы: жить нормальной жизнью`;
    case 2: return `🥇 Сегмент 1 — Мамы подростков\nВысокая платёжеспособность и острая боль — готовы платить за решение прямо сейчас.\n\n🥈 Сегмент 2 — Женщины 30–45 с тревожностью\nМассовый сегмент с постоянным спросом, хорошо реагируют на контент в соцсетях.\n\n🥉 Сегмент 3 — Пары в кризисе\nВысокий средний чек, часто приходят парой — двойная оплата.`;
    case 4: return `Подсегмент 1 — Мамы подростков с закрытостью ребёнка\nКогда: сын/дочь перестал разговаривать\nХочу: понять причину и помочь\nЧтобы: сохранить доверие\n\nПодсегмент 2 — Мамы подростков с агрессией\nКогда: постоянные конфликты дома\nХочу: прекратить ссоры\nЧтобы: жить в мире\n\nПодсегмент 3 — Мамы подростков с депрессией у ребёнка\nКогда: ребёнок грустит и замкнулся\nХочу: вернуть радость\nЧтобы: ребёнок был счастлив\n\nПодсегмент 4 — Мамы подростков с проблемами в школе\nКогда: падают оценки, прогулы\nХочу: разобраться в причинах\nЧтобы: ребёнок нормально учился\n\nПодсегмент 5 — Мамы подростков с зависимостью от телефона\nКогда: ребёнок не отрывается от экрана\nХочу: установить границы\nЧтобы: ребёнок жил реальной жизнью`;
    case 6: return `• Хочу наконец поговорить с ребёнком по душам\n• Хочу понять, что происходит у него внутри\n• Хочу перестать злиться и кричать\n• Хочу научиться слушать без осуждения\n• Хочу чтобы ребёнок сам приходил ко мне с проблемами\n• Хочу не чувствовать себя плохой мамой\n• Хочу методы которые реально работают\n• Хочу восстановить близость как раньше\n• Хочу понять подростковую психологию\n• Хочу чтобы дома была спокойная атмосфера\n• Хочу помочь ребёнку без психолога "по записи"\n• Хочу конкретный план действий`;
    case 7: return `1. Как наладить отношения с подростком который ни во что не ставит\n2. Почему ребёнок стал агрессивным и что делать\n3. Как разговаривать с подростком который молчит\n4. Что делать если подросток грубит и не слушается\n5. Как вернуть близость с ребёнком в 14–16 лет\n6. Ребёнок закрылся — нормально ли это и как помочь\n7. Как перестать срываться на подростке\n8. Нужен ли психолог подростку или достаточно мамы\n9. Как установить границы с подростком без скандала\n10. Почему подросток врёт и как восстановить доверие`;
    case 9: return `• Почему я не могу справиться с собственным ребёнком?\n• Я плохая мать раз у нас такие отношения?\n• Он меня ненавидит — или это пройдёт?\n• Когда это закончится или так будет всегда?\n• Я слишком давила на него в детстве?\n• Правильно ли я вообще воспитываю?\n• Почему другие мамы справляются а я нет?\n• Если сейчас не исправить — останется ли шрам на всю жизнь?\n• Зачем я вообще родила если теперь так больно?`;
    case 10: return `• Хотят чтобы ребёнок сам пришёл и сказал «мам, давай поговорим»\n• Мечтают о вечерах где смеются вместе а не молчат\n• Хотят быть той мамой о которой дети вспоминают с теплом\n• Хотят понять ребёнка так же хорошо как понимали в 5 лет\n• Мечтают что ребёнок будет делиться с ними своими тайнами\n• Хотят чтобы дом был безопасным местом для всех`;
    case 11: return `${sub} — клиент получает инструменты восстановления доверительного контакта с подростком и спокойствие вместо постоянного страха «сказать не то».`;
    case 12: return `Больше всего меня бесит то, что я не понимаю — что происходит с моим ребёнком. Я стараюсь, я читаю статьи, я пытаюсь говорить «по-новому» — а в ответ стена. Молчание. Или хуже — «отстань». Я хожу вокруг него как на цыпочках, боюсь лишний раз спросить как дела. Раньше мы были близкими. А теперь я чужой человек в его комнате. И самое страшное — я не знаю что я сделала не так. Может слишком контролировала? Слишком давила? Или наоборот — слишком много позволяла? Каждый вечер я ложусь спать с этим ощущением что упускаю что-то важное. Что время идёт. Что ещё пару лет — и он уедет, и этот шанс будет упущен навсегда.`;
    default: return `[Mock] Шаг ${stepId} — анализ для сегмента «${seg}»`;
  }
}
    ...
    for (const step of STEPS) {
      ...
      if (step.type === 'auto') {
        ...
        try {
          ...
          console.warn('[AI audience step', step.id, '] falling back to mock:', err);
          content = getMockContent(step.id, answers);
          toast('AI недоступен — используются демо-данные', { icon: '📋', duration: 3000 });
        }
        ...
        if (options.length < 2) {
          console.warn(`[Audience step ${step.id}] no usable options — using mock fallback`);
          const mockText = getMockContent(step.id === 3 ? 2 : step.id === 5 ? 4 : 7, answers);
          options = parseChoiceOptions(step.id, mockText).filter((o) => o.length > 3).slice(0, 3);
          if (options.length < 2) {
            options = step.id === 3

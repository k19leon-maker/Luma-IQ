...
function CheckIcon({ color = '#22c55e', size = 11 }: { color?: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
...
function MetricCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div style={{ flex: 1, borderRadius: 10, padding: '16px 20px', backgroundColor: '#F5F4F0' }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 700, color: '#D4A847', lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 12, color: '#999', marginTop: 4 }}>{sub}</div>
    </div>
  );
}

// ─── Product card (all filled) ────────────────────────────────────────────────

const PRODUCT_META = {
  MAIN: { label: 'Основной продукт' },
  MINI: { label: 'Мини-продукт'     },
  FREE: { label: 'Лид-магнит'       },
} as const;

function ProductCard({ product, type }: { product?: ApiProduct; type: 'MAIN' | 'MINI' | 'FREE' }) {
  ...
  return (
    <div style={{ borderRadius: 10, padding: '16px', display: 'flex', flexDirection: 'column', gap: 8, backgroundColor: '#F5F4F0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }}>
        ...
        <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, backgroundColor: '#dcfce7', color: '#166534' }}>
          <CheckIcon color="#166534" size={9} /> Готово
        </span>
      </div>
    </div>
  );
...
function MainContent({ products, onGenerateContent }: { products: ApiProduct[]; onGenerateContent: () => void }) {
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: '#fff', overflowY: 'auto' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 32px', borderBottom: '1px solid #F0EEE8', flexShrink: 0 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#888', margin: '4px 0 0' }}>100% выполнено · Проект упакован</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={onGenerateContent} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', backgroundColor: '#D4A847', color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
      ...
      <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 14 }}>
          <MetricCard label="Стратегия" value="7/7"                       sub="разделов"      />
          <MetricCard label="Продуктов" value="3/3"                       sub="настроено"     />
        ...
        <div style={{ borderRadius: 12, backgroundColor: '#1a1a1a', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 13.5, fontWeight: 600, color: '#D4A847', margin: 0, lineHeight: 1.4 }}>
              Проект полностью упакован! 🎉
            </p>
            <p style={{ fontSize: 12, color: '#888', margin: '3px 0 0', lineHeight: 1.4 }}>
              Все разделы заполнены — AI готов генерировать контент, посты и сценарии
            </p>
          </div>
          <button
        ...
        <div>
          ...
          <div style={{ borderRadius: 10, padding: '20px', display: 'flex', gap: 20, backgroundColor: '#F5F4F0' }}>
            <div style={{ flex: 3, minWidth: 0 }}>
              <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '1.5px', textTransform: 'uppercase', color: '#999', marginBottom: 4 }}>Сегмент · Подсегмент</div>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: '#1a1a1a', margin: '0 0 6px', lineHeight: 1.3 }}>
                {segment ? `${segment}${subsegment ? ' · ' + subsegment : ''}` : 'Данные из стратегии'}
              </h3>
              <p style={{ fontSize: 13, color: '#888', margin: '0 0 14px', lineHeight: 1.6 }}>
                {audienceData.answers.wants ?? 'Анализ целевой аудитории завершён'}
              </p>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                {['Психолог', 'Онлайн', 'Личная практика'].map((tag) => (
                  <span key={tag} style={{ padding: '4px 12px', borderRadius: 20, fontSize: 11.5, fontWeight: 500, backgroundColor: '#fff', color: '#888', border: '1px solid #E5E3DC' }}>{tag}</span>
                ))}
              </div>
              {/* 100% gold bar */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ flex: 1, height: 6, borderRadius: 3, backgroundColor: '#E5E3DC', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 3, backgroundColor: '#D4A847', width: '100%' }} />
                </div>
                <span style={{ fontSize: 11, color: '#D4A847', flexShrink: 0, fontWeight: 600 }}>7 из 7</span>
              </div>
            </div>
            ...
            <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: '#22c55e', fontWeight: 500 }}>Заполнено ✓</div>
              </div>
              <div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            ...
            <ProductCard product={productMini} type="MINI" />
            <ProductCard product={productFree} type="FREE" />
          </div>
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <button onClick={() => navigate('/content-plan')} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#D4A847', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>Весь план →</button>
          </div>
          ...
          {contentRows.length > 0 ? (
            ...
            <div style={{ borderRadius: 10, border: '1.5px dashed #E5E3DC', padding: '28px', textAlign: 'center', color: '#bbb', fontSize: 13 }}>
              Нет записей — перейдите в Контент-план чтобы добавить материалы
            </div>
          )}
        </div>
...
export default function DashboardComplete({ products, onGenerateContent }: DashboardCompleteProps) {
  const navigate = useNavigate();
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, height: '100%', overflow: 'hidden' }}>
      <MainContent
        products={products}
        onGenerateContent={() => { onGenerateContent?.(); navigate('/content-plan'); }}
      />
    </div>
  );
}
